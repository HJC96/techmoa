...
### 2026-02-21 (23차 프록시 기반 아키텍처 개선)
- **리버스 프록시 도입**: Nginx(Frontend 컨테이너)를 리버스 프록시로 설정하여 `/api` 요청을 백엔드로 전달하도록 구조 변경
- **환경 독립성 확보**: 
  - 프론트엔드 코드에서 하드코딩된 서버 IP 및 빌드 타임 환경변수(`VITE_API_BASE_URL`) 제거
  - `frontend/src/api/client.ts`에서 상대 경로(`/api`) 사용
- **개발 환경 최적화**: `vite.config.ts`에 `proxy` 설정을 추가하여 로컬 개발 시에도 동일한 API 경로 사용 가능하게 구성
- **보안 및 CORS 해결**: 프론트와 백엔드가 동일 도메인(Nginx 80포트)으로 통합됨에 따라 브라우저의 CORS 제약 해결 및 백엔드 포트(8080) 직접 노출 방지 기반 마련
